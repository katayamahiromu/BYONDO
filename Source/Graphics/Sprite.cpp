	...
	float r, float g, float b, float a) const
{
...

	{
		...
		immediate_context->OMSetBlendState(blendState.Get(), nullptr, 0xFFFFFFFF); // 透過
...
void Sprite::Render_Add(ID3D11DeviceContext* immediate_context,
	float dx, float dy,
	float dw, float dh,
	float sx, float sy,
	float sw, float sh,
	...
	float r, float g, float b, float a) const
{
		...
		// スプライトの中心で回転させるために４頂点の中心位置を任意の位置出来るように調整
		// NDC座標上？
		float mx = dx + dw * (cx / dw);
		float my = dy + dh * (cy / dh);
		for (auto& p : positions)
		{
			p.x -= mx;
			p.y -= my;
