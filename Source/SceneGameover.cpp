...
{    //スプライト初期化
	...
	lose_voice_1 = Audio::Instance().LoadAudioSource("Data/Sound/lose voice 1.wav");
	lose_voice_2 = Audio::Instance().LoadAudioSource("Data/Sound/lose voice 2.wav");
	lose_voice_3 = Audio::Instance().LoadAudioSource("Data/Sound/lose voice 3.wav");
	lose_voice_4 = Audio::Instance().LoadAudioSource("Data/Sound/lose voice 4.wav");
	...
	{
	case 0:
		lose_voice_1->Play(false);
		break;
	case 1:
		lose_voice_2->Play(false);
		break;
	}
}
...
void SceneGameover::Render()
{
	...
	//2Dスプライト描画
	{
		...
		float textureWidth = static_cast<float>(gameover->GetTextureWidth());
		float textureHeight = static_cast<float>(gameover->GetTextureHeight());
...
void SceneGameover::Scene_Change()
{
	effect = true;
...
void SceneGameover::Update_intarval(float elapsedTime)
{
	if (intarval >= 0) {
	...
	else
	{
		...
		else
		{
			...
			{
			case 0:
				lose_voice_3->Play(false);
				break;
			case 1:
				lose_voice_4->Play(false);
				break;
			}
		}
	}
